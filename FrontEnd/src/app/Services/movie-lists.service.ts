...
export class MovieListsService {
  ...
  public onBook: boolean = false;
  public inputTicket: number = 0;
  public getMovieId: number = 0;
  public getShowId: number = 0;
  public getTheaterId: number = 0;
  public seatsList: any = [];
  public seatSelected: any = [];
  public userBookings: any;
  public userMovieId:any=[];
  public hel:any;
  ...
  getTickets(movieId: number, theaterId: number, showId: number) {
    this.http.get('https://localhost:7220/api/Tickets/' + movieId + '/' + theaterId + '/' + showId).subscribe((data: any) => {
      ...
      this.getTheaterId = theaterId;
      this.getShowId = showId;
    })
  }

  getSeatIDs(movieId: number, theaterId: number, showId: number) {
    this.http.get('https://localhost:7220/api/Tickets/getseats/' + movieId + '/' + theaterId + '/' + showId).subscribe(data => {
      this.seatsList = data;
    })
  }
  getSeatstatus(movieId: number, theaterId: number, showId: number, seatId: number) {
    this.http.get('https://localhost:7220/api/Tickets/seats/' + movieId + '/' + theaterId + '/' + showId + '/' + seatId).subscribe(data => {
  ...
  postUserBookings(movieId: number, theaterId: number, showId: number, seatId: number) {
    this.http.get('https://localhost:7220/api/Tickets/myBookings/' + movieId + '/' + theaterId + '/' + showId + '/' + this.loggedUser.id + '/' + seatId).subscribe(data => {
    })
  }

  postMovie(movieName:string,language:string,moviePath:string)
  {
     this.http.get('https://localhost:7220/api/Movies/AddMovie/addMovie?'+'name='+movieName+'&language='+language+'&moviePath='+moviePath).subscribe(data => {

     })
  }

  postTheater(theaterName:string,theaterLocation:string)
  {
    this.http.get('https://localhost:7220/api/Theaters/PostTheater/theaters?name='+theaterName+ '&location=' + theaterLocation).subscribe(data => {

    })
  }

  postShow(movieId:number,theaterId:number,showId:number)
  {
    this.http.get('https://localhost:7220/api/Addshows?movieId='+movieId+'&theaterId='+theaterId+'&showId='+showId).subscribe(data => {

    })
  }

  // getUserBookings() {
  //   this.http.get('https://localhost:7220/api/Tickets/getmyBookings/' + this.loggedUser.id).subscribe(data => {
  //     console.log(data);
  ...
  //         this.userMovieId.push(data);
  //         console.log(this.userMovieId)
  //       });
  //     }
  //   })
  // }
